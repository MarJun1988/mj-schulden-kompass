...
- Fehlgeschlagene E-Mails bleiben persistent in einer Retry-Queue und werden automatisch erneut versendet.
- Prometheus und Grafana zeigen GraphQL-Anfragen, Fehler, Latenzen und den Zustand der Mail-Queue.
- Loki und Grafana Alloy sammeln Docker-Logs aus Backend, Frontend, Docs, Edge und den Infrastruktur-Containern.
- Das Grafana-Dashboard filtert Logs nach Docker-Service, Log-Level und Freitext.

### Geaendert

- Testing und Production koennen den vollstaendigen Monitoring-Stack als Compose-Profil starten.
- SMTP-Timeouts, Retry-Intervall, maximale Versuche und Batch-Groesse sind konfigurierbar.
- Das Grafana-Provisioning funktioniert auch mit bereits vorhandenen Grafana-Daten.
