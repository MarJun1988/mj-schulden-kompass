...
export type DebtCreditEntryType = "GRANT" | "PAYMENT" | "TRANSFER";
...
interface TransferDebtCreditData {
  transferDebtCredit: DebtCredit;
}
...
export const useDebtsStore = defineStore("debts", {
  ...
  actions: {
    ...
    async transferCredit(input: { creditId: string; amount: number; note?: string | null }) {
      const payload = await gql<TransferDebtCreditData>(
        `
          mutation TransferDebtCredit($input: TransferDebtCreditInput!) {
            transferDebtCredit(input: $input) {
              ${creditFields}
            }
          }
        `,
      ...
      const credit = payload.data?.transferDebtCredit;
      if (credit) {
        this.credits = [credit, ...this.credits.filter((item) => item.id !== credit.id)];
        await this.fetchDebts();
      }
