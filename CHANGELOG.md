...
- Fehlgeschlagene E-Mails werden persistent gespeichert und automatisch in einem konfigurierbaren Intervall erneut versendet.
- SMTP-Verbindungs-, Begruessungs- und Socket-Timeouts verhindern dauerhaft blockierte Mail-Anfragen.
- Das Backend stellt Prometheus-Metriken fuer GraphQL-Anfragen, Fehler, Laufzeiten und Mail-Zustaende bereit.
- Grafana wird mit einem SchuldKompass-Dashboard fuer Requests, Fehler, Latenzen, Mail-Queue und Docker-Logs ausgeliefert.
- Loki und Grafana Alloy sammeln und speichern die Logs aller Docker-Container fuer sieben Tage.
- Docker-Logs lassen sich in Grafana nach Service, Log-Level und Freitext filtern.

### Geaendert

- Local-, Testing- und Production-Compose enthalten den vollstaendigen Monitoring-Stack.
- Der Testing-Deploy startet Prometheus, Loki, Alloy und Grafana automatisch mit.
- Das Grafana-Provisioning aktualisiert bestehende Installationen ohne Loeschen des Grafana-Volumes.
- Monitoring-, Mail-Retry- und SMTP-Timeout-Einstellungen sind ueber Umgebungsvariablen konfigurierbar.
