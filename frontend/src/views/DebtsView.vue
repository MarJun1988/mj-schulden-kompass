...
const creditTransferDialogVisible = ref(false);
const creditSaving = ref(false);
const creditTransferSaving = ref(false);
const selectedCredit = ref<DebtCredit | null>(null);
const transferCreditTarget = ref<DebtCredit | null>(null);
...
const canTransferCredit = computed(() =>
  Boolean(
    transferCreditTarget.value &&
    transferCreditForm.value.amount &&
    transferCreditForm.value.amount > 0 &&
    transferCreditForm.value.amount <= transferCreditTarget.value.balance,
  ),
);
...
const canManageCredit = (credit: DebtCredit) => credit.creator.id === auth.user?.id;

const creditEntryLabel = (type: string) => {
  if (type === "GRANT") {
    return "Erhalten";
  }

  if (type === "TRANSFER") {
    return "Zurücküberwiesen";
  }

  return "Verwendet";
};

const creditEntryToneClass = (type: string) =>
  type === "GRANT" ? "text-emerald-700" : "text-rose-700";

const creditEntrySign = (type: string) => (type === "GRANT" ? "+" : "-");
...
const openCreditTransferDialog = (credit: DebtCredit) => {
  transferCreditTarget.value = credit;
  transferCreditForm.value = {
    amount: null,
    note: "",
  };
  creditTransferDialogVisible.value = true;
};

const closeCreditTransferDialog = () => {
  creditTransferDialogVisible.value = false;
  transferCreditTarget.value = null;
  transferCreditForm.value = {
    amount: null,
    note: "",
  };
...
const submitCreditTransfer = async () => {
  if (!transferCreditTarget.value || !canTransferCredit.value || !transferCreditForm.value.amount) {
    return;
  }

  const credit = transferCreditTarget.value;
  creditTransferSaving.value = true;

  try {
    await store.transferCredit({
      creditId: credit.id,
      amount: transferCreditForm.value.amount,
      note: transferCreditForm.value.note.trim() || null,
    });
    closeCreditTransferDialog();
    toast.add({
      severity: "success",
      summary: "Rücküberweisung gespeichert",
      detail: `Das Guthaben für ${credit.debtorName} wurde reduziert.`,
      life: 3500,
    });
  } catch (error) {
    toast.add({
      severity: "error",
      summary: "Rücküberweisung nicht gespeichert",
      detail:
        error instanceof Error
          ? error.message
          : "Die Rücküberweisung konnte nicht gespeichert werden.",
      life: 4500,
    });
  } finally {
    creditTransferSaving.value = false;
  }
};
...
<template>
  <section class="grid min-w-0 max-w-full gap-6">
        ...
        <div class="w-full min-w-0 overflow-x-auto">
          ...
          >
            <Column header="Typ" style="width: 10rem">
              <template #body="{ data }">
                <span :class="['font-medium', creditEntryToneClass(data.type)]">
                  {{ creditEntryLabel(data.type) }}
                </span>
              </template>
            </Column>
            ...
            <Column header="Betrag" style="width: 8rem" body-class="text-right">
              <template #body="{ data }">
                <span :class="['whitespace-nowrap font-semibold', creditEntryToneClass(data.type)]">
                  {{ creditEntrySign(data.type) }}{{ currency.format(data.amount) }}
                </span>
              </template>
            </Column>
            <Column header="Info">
    ...
    <Dialog
      v-model:visible="creditTransferDialogVisible"
      modal
      header="Guthaben zurücküberweisen"
      :style="{ width: 'min(92vw, 30rem)' }"
      @hide="closeCreditTransferDialog"
    >
      <form
        id="transfer-credit-form"
        class="mt-3 grid gap-5"
        @submit.prevent="submitCreditTransfer"
      >
        <p class="text-sm leading-6 text-stone-600">
          Erfasse hier eine Überweisung an den Schuldner. Der Betrag wird vom vorhandenen Guthaben
          abgezogen und als Rücküberweisung in der Guthaben-Info gespeichert.
        </p>
        <div class="rounded-md border border-stone-200 bg-stone-50 px-3 py-2 text-sm">
          <p class="font-medium text-stone-900">{{ transferCreditTarget?.debtorName }}</p>
          <p class="mt-1 text-stone-500">
            Verfügbar:
            {{ transferCreditTarget ? currency.format(transferCreditTarget.balance) : "" }}
          </p>
        </div>
        <FloatLabel :variant="formUi.floatLabelVariant">
          <InputNumber
            input-id="transfer-credit-amount"
            v-model="transferCreditForm.amount"
            class="w-full"
            mode="currency"
            currency="EUR"
            locale="de-DE"
            :min="0.01"
            :max="transferCreditTarget?.balance"
            required
          />
          <label for="transfer-credit-amount">Rücküberweisung *</label>
        </FloatLabel>
        <FloatLabel :variant="formUi.floatLabelVariant">
          <Textarea
            id="transfer-credit-note"
            v-model="transferCreditForm.note"
            class="w-full"
            rows="3"
            auto-resize
          />
          <label for="transfer-credit-note">Bemerkung</label>
        </FloatLabel>
      </form>
      <template #footer>
        <div class="mt-3 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
          <Button
            ...
            severity="secondary"
            label="Abbrechen"
            :disabled="creditTransferSaving"
            @click="closeCreditTransferDialog"
          />
          <Button
            type="submit"
            form="transfer-credit-form"
            label="Rücküberweisung speichern"
            icon="pi pi-send"
            :loading="creditTransferSaving"
            :disabled="creditTransferSaving || !canTransferCredit"
          />
        </div>
      </template>
    </Dialog>
    ...
    <Card v-if="visibleCredits.length">
      ...
      <template #content>
        ...
        >
          ...
          <Column header="Aktionen" style="width: 10rem" body-class="text-right">
            <template #body="{ data }">
              <div class="flex justify-end gap-2">
                <Button
                  v-if="canManageCredit(data) && data.balance > 0"
                  class="icon-action-button"
                  icon="pi pi-send"
                  label="Zurücküberweisen"
                  size="small"
                  severity="secondary"
                  title="Guthaben zurücküberweisen"
                  aria-label="Guthaben zurücküberweisen"
                  @click="openCreditTransferDialog(data)"
                />
                <Button
                  class="icon-action-button"
