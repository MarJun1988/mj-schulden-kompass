...
const visibleVersionLimit = 3;

const latestVersion = computed(() => versions.value.at(0) ?? null);
const hiddenVersionCount = computed(() => Math.max(versions.value.length - visibleVersionLimit, 0));
const visibleVersions = computed(() =>
  showOlderVersions.value ? versions.value : versions.value.slice(0, visibleVersionLimit),
);
              ...
              <Button
                ...
                :label="
                  showOlderVersions
                    ...
                    : `${hiddenVersionCount} ältere Versionen anzeigen`
                "
                :icon="showOlderVersions ? 'pi pi-chevron-up' : 'pi pi-chevron-down'"
                size="small"
                severity="secondary"
                outlined
                @click="showOlderVersions = !showOlderVersions"
              />
            </div>
